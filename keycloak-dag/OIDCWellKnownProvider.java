...
public class OIDCWellKnownProvider implements WellKnownProvider {
    ...
    public static final List<String> DEFAULT_GRANT_TYPES_SUPPORTED = list(OAuth2Constants.AUTHORIZATION_CODE, OAuth2Constants.IMPLICIT, OAuth2Constants.REFRESH_TOKEN, OAuth2Constants.PASSWORD, OAuth2Constants.CLIENT_CREDENTIALS, OAuth2Constants.DEVICE_CODE_GRANT_TYPE);
    ...
    public Object getConfig() {
        ...
        // Set device code endpoints
        config.setDeviceAuthorizationEndpoint(uriBuilder.clone().path(OIDCLoginProtocolService.class, "oauth2DeviceAuth").build(realm.getName(), OIDCLoginProtocol.LOGIN_PROTOCOL).toString());
        config.setDeviceVerificationEndpoint(uriBuilder.clone().path(OIDCLoginProtocolService.class, "oauth2DeviceVerificationCompleted").build(realm.getName(), OIDCLoginProtocol.LOGIN_PROTOCOL).toString());
